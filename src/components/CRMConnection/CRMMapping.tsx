...
export default function CRMUserMapping() {
  const [users, setUsers] = useState(['Johnny S.', 'Sarah D.', 'Emilia Z.', 'Gibso L.']);
  return (
        ...
        {users.map((item, index) => {
          return (
            <Flex gap={'sm'} align={'center'}>
              <Select w={'100%'} label={index === 0 && <Text color='gray'>SellScale User</Text>} placeholder='Pick value' data={users} />
              <Flex>
                <IconArrowsExchange color='gray' style={{ marginTop: `${index === 0 ? '26px' : '0px'}` }} />
              </Flex>
              <Select w={'100%'} label={index === 0 && <Text color='gray'>CRM User</Text>} placeholder='Pick value' data={users} />
            </Flex>
          );
        })}
      </Flex>
