...
import { IconSearch, IconAdjustmentsFilled, IconInfoCircle, IconClock, IconStar, IconBellOff } from "@tabler/icons-react";
...
const StatusSelectItem = forwardRef<HTMLDivElement, StatusSelectItemProps>(({ count, label, ...others }: StatusSelectItemProps, ref) => (
  <div ref={ref} {...others}>
    ...
    <>
      <Flex
        ...
        sx={(theme) => ({
          ...
          borderRight: props.opened ? `3px solid ${theme.colors.blue[theme.fn.primaryShade()]}` : `3px solid ${theme.white}`,
        })}
      >
        <div style={{ flex: 0 }}>
        ...
        <div style={{ flexGrow: 1, position: "relative" }}>
          <Stack spacing={0}>
            <Group position="apart" sx={{ flexWrap: "nowrap" }}>
              ...
              <Tooltip label={convertDateToLocalTime(new Date(props.latest_msg_time))} openDelay={500}>
            ...
            {adminData?.role === "ADMIN" && (
              <Flex align="center" gap="xs">
                <Avatar size="sm" radius="xl" src={proxyURL(props.client_sdr_img_url)} color={valueToColor(theme, props.client_sdr_name)}>
          ...
          {props.default_channel && (
            <Box sx={{ position: "absolute", top: 15, right: 0 }}>
              {props.default_channel === "LINKEDIN" ? (
                <ActionIcon variant="transparent" color="blue" radius="xl" size="sm" aria-label="LinkedIn">
                  <IconBrandLinkedin size="1.0rem" />
                </ActionIcon>
              ) : (
                <ActionIcon variant="transparent" color="yellow" radius="xl" size="sm" aria-label="Email">
          ...
          {props.snoozed_until && new Date(props.snoozed_until) > new Date() && (
            <Tooltip label={`Snoozed until ${convertDateToLocalTime(new Date(props.snoozed_until))}`} withArrow withinPortal>
              <Flex
                align={"center"}
                gap={"0.25rem"}
                sx={{
...
export default function ProspectList(props: { prospects: ProspectShallow[]; isFetching: boolean; all?: boolean }) {
  ...
  let prospects =
    props.prospects
      ...
      .sort(
        (a, b) =>
          _.findIndex(filterSelectOptions, (o) => o.value === a.linkedin_status) - _.findIndex(filterSelectOptions, (o) => o.value === b.linkedin_status) ||
  ...
  useEffect(() => {
    if (displayProspects.length > 0) {
      if (!openedProspectId || openedProspectId < 0 || !displayProspects.find((p) => p.id === openedProspectId)) {
  ...
  return (
    <div>
                ...
                return (
                  <div key={i}>
                    ...
                    {displayProspects.map((prospect, i: number) => (
                      <div key={i}>
                        {filterSelectValue === "ALL" && (!displayProspects[i - 1] || prospect.linkedin_status !== displayProspects[i - 1].linkedin_status) && (
                        ...
                        <Box
                          sx={{
                            position: "relative",
                            display: prospect.name.toLowerCase().includes(searchFilter.toLowerCase()) ? "visible" : "none",
                          }}
                        >
                          <Container
                            p={0}
                          ...
                          {prospect.in_purgatory && (
                            <Tooltip label={`Snoozed until ${convertDateToLocalTime(new Date(prospect.purgatory_until))}`} withArrow withinPortal>
                              <Flex
                                align={"center"}
                                gap={"0.25rem"}
                                sx={{
      ...
      <InboxProspectListFilter open={filterModalOpen} setOpen={setFilterModalOpen} filters={filtersState} setFilters={setFiltersState} all={props.all} />
    </div>
  );
}
