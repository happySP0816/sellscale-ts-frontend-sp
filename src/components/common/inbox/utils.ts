...
export function labelizeStatus(substatus: string) {
  return _.startCase(substatus.replaceAll('_', ' ').toLowerCase());
}
...
export function populateInboxNotifs(prospects: Prospect[]) {
  ...
  recommendedProspects = recommendedProspects.filter(
    (p) =>
      !p.li_is_last_message_from_sdr || isWithinLastXDays(new Date(p.li_last_message_timestamp), 3)
  );
  recommendedProspects = recommendedProspects.filter((p) => {
    return prospectStatuses.find((option) => option.value === p.linkedin_status);
  });
  recommendedProspects = recommendedProspects.filter(
    (p) => !(p.hidden_until ? new Date(p.hidden_until).getTime() > new Date().getTime() : false)
  );
