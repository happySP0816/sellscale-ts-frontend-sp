...
export default function ProspectDetailsCRMSync(props: {
  ...
  useEffect(() => {
    const lead_synced = (props.crmSync.lead_sync && props.prospect?.merge_lead_id) as boolean;
    ...
    setsynced(lead_synced || contact_synced || account_synced || opportunity_synced);
    setSyncableModels(
      [
    ...
    setSelectedStage(existingStageMapping[props.prospect.overall_status] || null);
  }, [props.prospect]);
  ...
  const getCRMStages = async () => {
    ...
    if (data.status === 'success') {
      ...
      setSelectedStage(data.data.mapping[props.prospect.overall_status] || null);
    }
  };
    ...
    <Box style={{ flexBasis: '15%' }} p={10} px={'md'} mb={'sm'}>
      ...
      <Text size='xs' mt='xs'>
        ...
        >
          ...
          >
            {props.prospect?.merge_opportunity_id ? '✅ Opportunity' : '☑️ Opportunity'}
          </span>
        </Tooltip>
      </Text>
      {synced && (
      ...
      {synced && (
        <Text color='gray' fw={500} size={'sm'}>
          Opportunity Value:{' '}
          {`$${props.prospect?.contract_size || userData.client.contract_size || ' Not Set'}`}
        </Text>
      )}
      <Modal
        opened={CRMOpened}
      ...
      >
        <Text>
          {`Automatically create a ${syncableModels.join(', ')} in ${props.crmSync.crm_type} for ${
        ...
        <Flex direction={'column'} gap={'sm'}>
          ...
          {props.crmSync.account_sync && <Checkbox label='Create Account' checked />}
          {props.crmSync.contact_sync && <Checkbox label='Create Contact' checked />}
          {props.crmSync.opportunity_sync && (
            <>
              ...
              <Flex>
                <Select
                  ...
                  value={
                    selectedStage ||
                    (existingStageMapping && existingStageMapping[props.prospect.overall_status]) ||
                    ''
                  }
                  onChange={(value) => {
          ...
          <Flex align={'center'} gap={'sm'} mt={'xl'}>
            <Button variant='outline' size='md' fullWidth color='red' onClick={CRMClose}>
              Cancel
            </Button>
            <Button
              ...
              disabled={props.crmSync.opportunity_sync && selectedStage === null}
            >
              Sync
            </Button>
          </Flex>
