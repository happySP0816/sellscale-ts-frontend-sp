...
import { ActionIcon, Badge, Button, Flex, NavLink, Progress, Select, Text, useMantineTheme } from '@mantine/core';
import { IconBolt, IconCalendar, IconChevronLeft, IconChevronRight, IconExternalLink, IconLetterT, IconLoader } from '@tabler/icons';
...
export default function ProspectUploadHistory() {
  ...
  const [uploadHistoryData, setUploadHistoryData] = useState<UploadHistoryDataType[]>([]);
      ...
      <DataGrid
        ...
        columns={[
          ...
          },
          {
            ...
            enableResizing: true,
            cell: (cell) => {
              const { upload_size, uploads_completed, upload_source } = cell.row.original;
              ...
              return (
                <Flex direction={'column'} align={'center'} justify={'center'} w={'100%'} h={'100%'} py={'sm'}>
                  <Flex w={'100%'} align={'center'} justify={'flex-start'} gap={'8px'} px={'xs'}>
                    ...
                    />
                    <Text color={Math.round((uploads_completed / upload_size) * 100) >= 100 ? 'green' : '#228be6'} fw={600}>
