...
export default function WebTrafficRouting() {
  const script = `<script>
    ! function () {var sellscale = window.sellscale = window.sellscale | | [];
    if (sellscale.invoked) return; sellscale.invoked = true; sellscale.methods = ["identify", "collect"];
    sellscale. factory = function (method) {return function () {var args = Array.prototype.slice.call(arguments);
    args.unshift(method);sellscale.push(args);return sellscale;};};
    for (var i = 0; i < sellscale.methods. length; i++) {var key = sellscale.methods[i]; sellscale[key] = sellscale. factory(key)
    sellscale. load = function (key) {var script = document.createElement("script");script.type = "text/javascript'
    script.src = "https://s3-us-west-2.amazonaws.com/b2bjsstore/b/"+ key +"/sellscale.js.gz";
    var first = document.getElementsByTagName("script") [0];
    first.parentNode. insertBefore(script, first);};
    sellscale.SNIPPET_VERSION ="1.0.1";sellscale.load("ZQ6J2RH1W86D");}();
    </script>`;
  ...
  return (
    <Card withBorder p={"lg"}>
      ...
      <Text size={"xs"} color="gray">
        ...
        website traffic & opportunites correlate
      </Text>
      <Divider mt={4} />
          ...
          >
            ...
            <Timeline.Item bullet="3" title="Test script">
              <Text c="dimmed" size="xs" fw={500}>
                This is to verify taht the script is live and firing properly by
