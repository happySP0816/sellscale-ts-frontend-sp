...
const LiInitialMessageAssets: React.FC<{
  linkedin_initial_message_id: number;
  ...
  useEffect(() => {
    fetchAssets();
    fetchAttachedAssets();
  }, [linkedin_initial_message_id]); // Re-run these functions if linkedin_initial_message_id changes
  ...
  const fetchAttachedAssets = () => {
    fetch(
      `${API_URL}/client/archetype/li_init_template/get_all_asset_mapping?linkedin_initial_message_id=${linkedin_initial_message_id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const addAsset = () => {
    fetch(`${API_URL}/client/archetype/li_init_template/create_asset_mapping`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + userToken,
  ...
  const removeAsset = (assetMappingId: number) => {
    fetch(`${API_URL}/client/archetype/li_init_template/delete_asset_mapping`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + userToken,
      ...
      body: JSON.stringify({
        linkedin_initial_message_to_asset_mapping_id: assetMappingId,
      }),
    })
      .then((response) => {
        if (response.ok) {
            ...
            <Card key={asset.mapping_id} withBorder w="100%">
              <Flex>
                <CloseButton
                  mr="xs"
                  mt="xs"
                  onClick={() => removeAsset(asset.mapping_id)}
                />
                <Box>
...
export default LiInitialMessageAssets;
