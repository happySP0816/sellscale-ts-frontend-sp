...
export default function SingleEmailCampaignBetaModal({
  context,
  id,
  innerProps,
}: ContextModalProps<{
  ...
  const [inputLinkedin, setInputLinkedin] = useState(false);
    ...
    <Paper style={{ position: "relative" }}>
            ...
            <Stack spacing={"sm"}>
              <Text fw={400} color="black" size={"sm"}>
                ...
                <span className="font-semibold text-black">
                  We have found some prospects that are already added to your prospect database. Please check the prospects that you want to overwrite and move
                  to your new segment/campaign.<br></br>
                  ...
                  columns={[
                    {
                      ...
                      cell: (cell: any) => <Text fw={500}>{cell.getValue()}</Text>,
                    },
                    {
                      accessorKey: "company",
                      header: () => (
                    ...
                    },
                    {
                      ...
                      cell: (cell: any) => <Badge color={icpFitToColor(cell.getValue())}>{icpFitToLabel(cell.getValue())}</Badge>,
                    },
                    {
      ...
      <Flex align={"center"} gap={"xs"} mt={6}>
        ...
        <Flex align={"center"} gap={"xs"} w={"100%"}>
          <Popover position="bottom" width={480} withArrow shadow="md" opened={opened}>
            <Popover.Target>
              <TextInput
                ...
                size="xs"
                value={toEmail}
                rightSection={toEmail.length > 0 && <IconCircleCheck fill="green" color="white" size={"1.2rem"} onMouseEnter={open} onMouseLeave={close} />}
              />
            </Popover.Target>
              ...
              <Stack>
                <Box>
                  <Flex align={"center"} justify={"space-between"} w={"100%"}>
                    ...
                    <Badge color="green" leftSection={<IconCircleCheck fill="#37B24D" color="#EBFBEE" className="mt-[6px]" size={"1rem"} />}>
                      valid
                    </Badge>
                  </Flex>
                ...
                <Box>
                  ...
                  <Text size={"sm"} color="gray" fw={400}>
                    The domain in name isn't used for webmails or for creating temporary email addresses.
                  </Text>
                  <Divider mt={"sm"} />
                </Box>
                <Box>
                  ...
                  <Text size={"sm"} color="gray" fw={400}>
                    MX records are presents for the domain and we can connect to the SMTP server these MX records point to.
                  </Text>
                  <Divider mt={"sm"} />
                </Box>
                <Box>
                  <Flex align={"center"} justify={"space-between"} w={"100%"}>
                    ...
                    <Badge color="green" variant="light" leftSection={<IconCircleCheck fill="#37B24D" color="#EBFBEE" className="mt-[6px]" size={"1rem"} />}>
                      valid
                    </Badge>
                  </Flex>
                  <Text size={"sm"} color="gray" fw={400}>
                    This email address exist and can receive emails.
                  </Text>
                </Box>
              </Stack>
            </Popover.Dropdown>
          </Popover>
          <Tooltip label="Find Email from Linkedin URL" withArrow>
            <Button variant="default" size="xs" onClick={() => setInputLinkedin(true)}>
              <IconBrandLinkedin size={"1.2rem"} color="white" fill="gray" />
            </Button>
          </Tooltip>
          <Button disabled variant="default" size="xs" onClick={() => setCcEmail((prev) => (prev ? null : "cc@example.com"))}>
            +CC
          </Button>
          <Button disabled variant="default" size="xs" onClick={() => setBccEmail((prev) => (prev ? null : "bcc@example.com"))}>
            +BCC
          </Button>
        </Flex>
      </Flex>
      {inputLinkedin && (
        <TextInput
          ...
          onChange={(e) => {
            setLinkedinSearch(e.currentTarget.value);
            console.log(isValidLinkedInUrl(e.currentTarget.value));
            if (isValidLinkedInUrl(e.currentTarget.value)) getEmailFromLinkedinURL(e.currentTarget.value);
          }}
          rightSection={
...
const ProspectDetailsCard = ({
  prospect,
  setProspect,
}: {
  prospect: IScraperProspect;
  setProspect: React.Dispatch<React.SetStateAction<IScraperProspect | null>>;
}) => {
  const theme = useMantineTheme();
  ...
  return (
    <Flex gap={0} wrap="nowrap" direction="column" bg={"white"} sx={{ borderLeft: "0.0625rem solid #dee2e6", borderRight: "0.0625rem solid #dee2e6" }}>
        ...
        <Flex align={"start"} gap={"md"}>
          ...
          <Box maw={"70%"} w={"100%"}>
            <Flex align={"center"} gap={"sm"}>
                ...
                <Text size={"lg"} fw={700}>
                  {((prospect?.first_name + " " + prospect?.last_name)?.length || 0) > 16 ? (
                    <Tooltip label={prospect?.first_name + " " + prospect?.last_name}>
                      <span>{(prospect?.first_name + " " + prospect?.last_name).substring(0, 16) + "..."}</span>
                    </Tooltip>
                  ) : (
              ...
              <ActionIcon onClick={handleEditToggle}>{isEditing ? <IconCheck color="green" /> : <IconPencil color="gray" />}</ActionIcon>
            </Flex>
              ...
              <Group noWrap spacing={10} mt={3}>
                ...
                {isEditing ? (
                  <TextInput
                    ...
                    onChange={(e) =>
                      handleChange("position_groups", [
                        {
                          ...editableProspect.position_groups[0],
                          profile_positions: [{ ...editableProspect.position_groups[0].profile_positions[0], title: e.currentTarget.value }],
                        },
                      ])
                    }
                    size="xs"
              ...
              <Group noWrap spacing={10} mt={3}>
                ...
                {isEditing ? (
                  <TextInput
                    ...
                    onChange={(e) =>
                      handleChange("position_groups", [
                        { ...editableProspect.position_groups[0], company: { ...editableProspect.position_groups[0].company, name: e.currentTarget.value } },
                      ])
                    }
                    size="xs"
                  />
                ) : (
                  <Text size="xs" component="a" target="_blank" rel="noopener noreferrer" href={prospect.position_groups?.[0]?.company?.url}>
              ...
              <Group noWrap spacing={10} mt={5}>
                ...
                {isEditing ? (
                  ...
                  <Text size="xs" component="a" href={`mailto:${prospect?.contact_info?.email}`}>
                    {prospect?.contact_info?.email} <IconExternalLink size="0.7rem" color={theme.colors.blue[6]} />
                  </Text>
                )}
              </Group>
            )}
