...
import { createContext, Dispatch, Fragment, Key, memo, SetStateAction, useEffect, useRef, useState, useMemo } from "react";
...
export default function SelinAI() {
  ...
  const needInputThreads = threads
    .filter(
      (thread) =>
        thread.status && thread.status !== "COMPLETE" && thread.status !== "CANCELLED" && thread.tasks && thread.tasks.some((task) => task.status === "BLOCKED")
    )
    .sort((a, b) => b.id - a.id);
  ...
  const handleNewMessage = (data: { message?: string; action?: any; device_id?: string; role: "user" | "assistant" | "system"; thread_id: string }) => {
    ...
    if (data.thread_id === roomIDref.current) {
      if (data.message) {
        ...
        setMessages((chatContent: MessageType[]) => [...chatContent, newMessage]);
  ...
  useEffect(() => {
    const fetchAndLoadMessages = async () => {
      ...
      if (sessionIdFromUrl) {
        ...
        // }
        const threadId = threads_loaded.find((thread: ThreadType) => thread.id === Number(sessionIdFromUrl))?.thread_id;
        const newUrl = `${window.location.pathname}?${urlParams.toString()}`;
        window.history.replaceState({}, document.title, newUrl);
        getMessages(threadId || "", parseInt(sessionIdFromUrl), threads_loaded, "PLANNER");
  ...
  const [intendedTaskChange, setIntendedTaskChange] = useState<
    "ACTIVE" | "COMPLETE" | "CANCELLED" | "PENDING_OPERATOR" | "BLOCKED" | "IN_PROGRESS" | undefined
  >(undefined);
  ...
  const supervisor_thread = threads.find((thread) => thread.is_supervisor_session);
  const [clientMemoryState, setClientMemoryState] = useState<string | undefined>(
    threads.find((thread) => thread.id === sessionIDRef.current)?.memory?.memory_line
  );
  ...
  const [memoryState, setMemoryState] = useState<any>(threads.find((thread) => thread.id === sessionIDRef.current)?.memory.memory_state);
  ...
  useEffect(() => {
    const handleSpanHover = (event: any) => {
      if (event.target && event.target.dataset.title) {
        const session = memoryState?.sessions.find((s: any) => s.title === event.target.dataset.title || "@" + s.title === event.target.dataset.title);
        if (session) {
  ...
  const memory = threads.find((thread) => thread.id === sessionIDRef.current)?.memory;

  const [clientMemoryStateUpdatedTime, setClientMemoryStateUpdatedTime] = useState<any>(memory?.memory_line_time_updated);
        ...
        <Card maw={"100%"} style={{ backgroundColor: "transparent" }} p={0}>
          {currentSessionId && (
            <Flex gap={"xl"}>
              ...
              <Paper withBorder h={"100%"} className={`transition-all duration-300 ${showSidebar ? "w-[380px]" : "w-[90px]"}`}>
                    ...
                    >
                      ...
                      <Popover.Dropdown>
                        <Flex justify="space-between" align="center" mb="xs">
                          ...
                          {memory?.session_mode && (
                            <Tooltip label={"Current Goal: " + memory.session_current_goal} withArrow>
                        ...
                        <Card withBorder mah={700} p="md" sx={{ overflow: "auto" }}>
                              ...
                              <HoverCard.Target>
                                <Text
                                  ...
                                  dangerouslySetInnerHTML={{
                                    __html: (clientMemoryState || "Click to add notes...")?.replace(/\n/g, "<br>"),
                                  }}
                                />
                              </HoverCard.Target>
                          ...
                          <Text align="right" size="12px" color="gray" mt="2px" ml="auto">
                            Last updated: {memory?.memory_line_time_updated ? moment.utc(memory?.memory_line_time_updated).local().fromNow() : "N/A"}
                          </Text>
                          <Flex mt="4px" mb="md">
                            <Tooltip label="Rewind to previous version" withArrow>
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setClientMemoryStateUpdatedTime(memory?.old_memory_line_time_updated);
                                  setMemoryStateChanged(true);
                                }}
                              >
                                ⏮
                            ...
                            >
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setClientMemoryStateUpdatedTime(memory?.memory_line_time_updated);
                                  setMemoryStateChanged(false);
                                  setMemoryLineEditMode(false);
                                }}
                              >
                            ...
                            {supervisor_thread && (
                              <Paper
                                ...
                                style={{
                                  ...
                                  display: "inline-block",
                                  // minWidth: "350px",
                                  backgroundColor: sessionIDRef.current === supervisor_thread.id ? "#d0f0c0" : "white", // Highlight if current thread
                                  borderColor: sessionIDRef.current === supervisor_thread.id ? "#00796b" : "#e6ebf0", // Change border color if current thread
                                }}
                                ...
                                onClick={() => {
                                  getMessages(supervisor_thread.thread_id, supervisor_thread.id);
                                  toggle();
                                }}
                                onMouseEnter={() => setHoverChat(supervisor_thread.id)}
                                ...
                                <Flex align={"center"} justify={"space-between"}>
                                  <Flex align={"left"} w={"100%"}>
                                    <Text
                                      ...
                                      size={"sm"}
                                    >
                                      {supervisor_thread.session_name || "Supervisor Session"}
                                    </Text>
                                  </Flex>
                                </Flex>
                              </Paper>
                                  ...
                                  >
                                    <Flex align={"center"} justify={"space-between"}>
                                          ...
                                          <Flex align={"center"} gap={"xs"}>
                                            <Tooltip label={thread.session_name}>
                                              <Text
                                                ...
                                                size={"sm"}
                                              >
                                                {thread.session_name.substring(0, 27) + (thread.session_name.length > 27 ? "..." : "") || "Untitled Session"}
                                              </Text>
                                            </Tooltip>
                                          </Flex>
                                          <Tooltip label="Edit session name" withArrow position="top">
                                            <ActionIcon
                                              variant="transparent"
                                              // color="blue"
                                              size={"sm"}
                                      ...
                                      {thread.status !== "COMPLETE" && thread.status !== "CANCELLED" && (
                                        <>
                                          <Tooltip label="Archive session" withArrow position="top">
                                            <ActionIcon
                                              variant="transparent"
                                              color="red"
                                              size={"sm"}
                                            ...
                                            >
                                              {thread.status !== "CANCELLED" && thread.status !== "COMPLETE" && <IconArchive size={"1rem"} />}
                                            </ActionIcon>
                                          </Tooltip>
                                        </>
                                      )}
                                    ...
                                    <Flex align={"center"} justify={"space-between"}>
                                      {editingIndex === index ? (
                                        ...
                                        <Flex align={"center"}>
                                          <Tooltip label="Needs more input" withArrow position="top">
                                            <Box mr="xs">
                                              <IconCircleCheck color="green" size={"1.2rem"} />
                                            </Box>
                                          </Tooltip>
                                          <Tooltip label={thread.session_name}>
                                            <Text
              ...
              <Flex gap="md" p="lg" className={`${showSidebar ? "w-[calc(100%-380px)]" : "w-[calc(100%-90px)]"}`}>
                <DragDropContext
                  onDragEnd={(result) => {
...
const SegmentChat = (props: any) => {
  ...
  return (
    <>
      <Paper withBorder shadow="sm" radius={"md"} w={props.chatfullSize ? "40%" : "100%"} h={"100%"} className=" transition-all duration-300">
                      ...
                      {message.type === "message" || message.type === "slack" ? (
                        <>
                          ...
                          <Flex
                            ...
                            style={{
                              width: messageRefs.current[index]?.offsetWidth || "85%",
                              marginBottom: "-10px",
                            }}
                          >
                          ...
                          <Flex
                            ...
                            ml={message.role === "user" ? "auto" : "0"}
                            style={{
                              backgroundColor: message.hidden_until ? "#ADD8E6" : message.role === "user" ? "#f7ffff" : "#fafafa",
                              borderRadius: "10px",
                          ...
                          >
                            ...
                            <Text color="gray" size={"xs"} ml={message.role === "user" ? "auto" : "0"}>
                              <Text color={message.hidden_until ? "white" : "gray"} size="xs" ml={message.role === "user" ? "auto" : "0"}>
                                {message.hidden_until ? (
                                  <>
                                    ...
                                    />{" "}
                                    {moment(message.hidden_until).format("MMMM D, h:mm A")}
                                  </>
                                ) : (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      {suggestedFirstMessage.map((message, index) => (
                        ...
                        >
                          <Flex align={"center"} gap={"xs"} className="transition-transform duration-300 transform hover:translate-x-2">
                            <ThemeIcon color={typeof message === "string" ? "grape" : "blue"} size={"xl"}>
                              <IconBrain size={"1.4rem"} />
                            </ThemeIcon>
                            <Text
                              color={typeof message === "string" ? "#E25DEE" : "blue"}
                              fw={600}
                              size={"sm"}
                ...
                {sendAsSelix && (
                  <>
                    ...
                    <Checkbox
                      ...
                      label={<img src={SlackLogo} alt="slack" width={20} height={20} />}
                      size="sm"
                      color="grape"
                    />
                    <Popover position="bottom" withArrow shadow="md" trapFocus opened={showSchedulePopup}>
                      <Popover.Target>
                        <Tooltip label="Schedule a send time into the future" withArrow withinPortal>
                          <Flex>
                            <Button onClick={() => setShowSchedulePopup((v) => !v)} size="xs">
...
const SelixControlCenter = ({
  ...
  return (
    <Paper withBorder shadow="sm" w={!chatfullSize ? "60%" : "100%"} radius={"md"}>
      <Modal
        opened={showICPModal}
        onClose={() => {
      ...
      <Flex px={"md"} py={"xs"} align={"center"} justify={"space-between"} gap={5} bg={"#E25DEE"} className=" rounded-t-md">
        ...
        {chatfullSize && (
          <ActionIcon variant="transparent" onClick={() => setChatfullSize(false)}>
...
export const PlannerComponent = ({
  ...
  const fetchSelixLogs = async (selixLogId: number | null = null): Promise<MemoryLog[]> => {
    try {
      ...
      l = l
        .filter((log) => {
          if (currentSessionId) {
            ...
            return log.show_in_session_memory && +log.session_id === +currentSessionId;
          } else {
  ...
  return (
    <Paper p={"sm"} radius={"sm"}>
      ...
      <Paper bg={"#fefafe"} my={"sm"} px={"sm"} py={8} style={{ borderColor: "#fadafc" }}>
        <Flex align={"center"} gap={"xs"} justify={"space-between"}>
          {currentProject && currentThread?.memory.campaign_id === currentProject?.id && (
            <Text size={"xs"} color="#E25DEE" fw={600}>
              Selix Tasks:{" "}
              <span className="font-medium text-gray-500">
                {currentProject.name}{" "}
                <a href={`/campaign_v2/${currentProject.id}`} target="_blank" rel="noopener noreferrer" style={{ marginLeft: "5px" }}>
      ...
      <Flex align={"center"} justify={"space-between"}>
          ...
          <Select
            ...
            data={logs.map((value, index) => {
              if (index === 0) {
                ...
                return {
                  value: "" + value.id,
                  label: moment.utc(value.created_date, "YYYY-MM-DD HH:mm:ss").tz("America/Los_Angeles").format("YYYY-MM-DD HH:mm:ss"),
                };
              }
            })}
        ...
        {selectedLog && (
          <Popover width={500} shadow={"md"} position={"bottom"} withinPortal>
            <Popover.Target>
              <Text ml={"auto"} size={"xs"} color="gray" sx={{ pointer: "cursor" }}>
                <Badge color="pink" variant="outline">
                  🧠
                </Badge>
              </Text>
                                          ...
                                          tasks.map((t, i) =>
                                            i === index
                                              ? {
                                                  ...
                                                  description: taskDraftDescription.current,
                                                }
                                              : t
                                          )
                                        );
...
const TaskRenderer = ({
  ...
  switch (task.widget_type) {
    ...
    case "COMPANY_SEGMENT":
      return <CompanySegmentReview selixSessionId={currentSessionId} connectedCompanySegmentId={currentThread?.memory?.company_segment_id} />;
    ...
    case "VIEW_SEQUENCE":
      return <SequencesV2 ref={sequencesV2Ref} showComponent={true} updateConnectionType={updateConnectionType} forcedCampaignId={currentProject?.id} />;
    default:
      return (
