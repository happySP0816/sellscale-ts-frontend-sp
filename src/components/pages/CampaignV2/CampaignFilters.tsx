  ...
  return (
    ...
    >
      ...
      >
        ...
        <ItemCollapse title={"Individual"} showNumber={false} color="purple">
        ...
        <ItemCollapse title={"Company"} showNumber={false} color={"purple"}>
          <Flex direction={"column"} gap={"4px"}>
            <ItemCollapse
            ...
            <ItemCollapse
              ...
              numberOfItem={company_size_start && company_size_end ? 1 : 0}
            >
        ...
        <ItemCollapse title={"AI Filters"} showNumber={false} color={"purple"}>
          <Switch
            onLabel="Individual"
            offLabel="Company"
            checked={viewIndividualAIFilters}
            onChange={(event) =>
              setViewIndividualAIFilters(event.currentTarget.checked)
            }
            size={"lg"}
          />
            ...
            <>
              ...
              <Button
                ...
                onClick={() => {
                  const key =
                    "aiind_" +
                    individual_ai_title.toLowerCase().split(" ").join("_");
          ...
          {individual_ai_filters.map((aiFilter, index) => {
            return (
              ...
              >
                <Flex direction={"column"} gap="4px">
                  <Title order={6}>{aiFilter.title}</Title>
                  <Badge variant={"filled"} color={"grape"}>
                    Individual
                  </Badge>
                  <Divider />
                  <Text size={"xs"}>{aiFilter.prompt}</Text>
                  <Checkbox
                    checked={dealbreakers.includes(aiFilter.key)}
                    onChange={(event) => {
                      setUpdatedIndividualColumns(
                        (prevState) => new Set([...prevState, aiFilter.key])
                      );
                      ...
                      if (event.currentTarget.checked) {
                        ...
                        setDealBreakers(
                          dealbreakers.filter((x) => x !== aiFilter.key)
                        );
                      }
                    }}
                    label={"Dealbreaker"}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setContactTableHeaders((prevState) =>
                        prevState.filter((item) => item.key !== aiFilter.key)
                      );
                      setDealBreakers((prevState) =>
                        prevState.filter((x) => x !== aiFilter.key)
                      );
