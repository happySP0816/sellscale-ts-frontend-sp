  ...
  const columns = [
    {
      ...
      cell: (cell: any) => {
        ...
        const totalProspectedChildren = sub_segments.reduce(
          (total: number, segment: any) => total + (segment.num_prospected || 0),
          0
        );
        const totalContactedChildren = sub_segments.reduce(
          (total: number, segment: any) => total + (segment.num_contacted || 0),
          0
        );
                    ...
                    <Flex direction='column' align={'left'} gap={'sm'} mt={3}>
                      <Flex direction='row' align={'left'} gap={'sm'} mt={3}>
                        ...
                        <Text size='10px' color={!isChild ? '#3B85EF' : 'grape'} fw={600}>
                          ...
                          {'in' + (isChild ? ' child segment' : ' segment')}
                        </Text>
                      </Flex>
                      <Flex direction='row' align={'left'} gap={'sm'} mt={3}>
                        {sub_segments && sub_segments.length > 0 && (
                          <>
                            <Progress
                              value={Math.round(
                                (totalContactedChildren / totalProspectedChildren + 0.0001) * 100
                              )}
                              w={140}
                              color='grape'
                            />
                            <Text size='10px' color='grape' fw={600}>
                              {`${
                                Math.round(
                                  (totalContactedChildren / totalProspectedChildren + 0.0001) * 100
                                ) || 0
                              }% ( ${totalContactedChildren} / ${totalProspectedChildren} ) in children segments`}
                            </Text>
                          </>
                        )}
                      </Flex>
    ...
    },
    {
      ...
      cell: (cell: any) => {
        ...
        const { contacts, filters, client_sdr, apollo_query, autoscrape_enabled } =
          cell.row.original;
                  ...
                  <>
                    ...
                    {showAutoDownloadFeature && (
                      ...
                      >
                        ...
                        {!doneScraping && !autoscrape_enabled && successfulScrapes > 0
                          ? ' - ' +
                            successfulScrapes +
                            '/' +
                            totalScrapes +
                            ' scraped (' +
                            Math.round((successfulScrapes / totalScrapes + 0.0001) * 100) +
                            '%)'
                          : ''}
                      </Button>
                    )}
                ...
                >
                  ...
                  {availableSegmentTags?.map((tag: { name: string; id: number }, index: number) => {
                    const isTagInSegment = segmentTags?.some(
                      (existingTag: { name: string }) => existingTag?.name === tag?.name
                    );
                    return (
                      tag && (
                            ...
                            <ActionIcon
                              ...
                              onClick={(e) => {
                                ...
                                openConfirmModal({
                                  ...
                                  children: (
                                    <Text size='sm'>
                                      Are you sure you want to permanently delete this tag? It will
                                      get removed from all locations.
                                    </Text>
                                  ),
                                  labels: {
                                    confirm: 'Delete Tag',
                    ...
                    <TextInput
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          ...
                          if (
                            newTagName !== '' &&
                            !availableSegmentTags?.some((tag) => tag.name === newTagName)
                          ) {
                            createSegmentTag(
                              userToken,
                              cell.row.original.id,
