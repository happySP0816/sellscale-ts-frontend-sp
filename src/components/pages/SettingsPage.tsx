    ...
    <Box p={20}>
      ...
      >
        <Tabs.List h={'fit-content'}>
          <Title
            color={['brain', 'messages', 'filters'].includes(currentTab) ? 'blue' : 'gray'}
            order={5}
            mt='lg'
            mb='xs'
          >
          ...
          <Title
            color={
              ['linkedin', 'email', 'slack', 'scheduling'].includes(currentTab) ? 'blue' : 'gray'
            }
            order={5}
            mt='lg'
            mb='xs'
        ...
        <Tabs.Panel value='filters' pl='xs'>
          <Group noWrap>{currentTab === 'filters' && <DoNotContactFiltersPage />}</Group>
        </Tabs.Panel>
