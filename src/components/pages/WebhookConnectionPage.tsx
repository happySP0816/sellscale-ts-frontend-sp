...
const eventTypes = [
  {
    label: "On Demo Set",
    value: "on_demo_set",
    subtitle: "When a demo is set with a prospect",
    disabled: false,
    apiEndpoint: "/set_on_demo_set_webhook",
    getEndpoint: "/get_client_webhooks",
  },
  {
    label: "On Reply",
    value: "on_reply",
    subtitle: "When a prospect replies to an email",
    disabled: false,
    apiEndpoint: "/set_on_reply_webhook",
    getEndpoint: "/get_client_webhooks",
  },
];
...
const examplePayload = `{
  ...
  "prospect_direct_link": "https://app.sellscale.com/prospects/277151"
}`;
...
const WebhookConnectionPage = () => {
    ...
    const fetchWebhooks = async () => {
      try {
        const response = await fetch(
          `${API_URL}/webhooks/get_client_webhooks`,
          {
            method: "GET",
            headers: {
        ...
        setWebhookUrls((current: any) => ({
          ...current,
          on_demo_set: data.on_demo_set || "",
        }));
        setIsLoading(false);
      } catch (error) {
  ...
  const saveWebhookUrl = async (eventType: string) => {
    ...
    if (event && event.apiEndpoint) {
      try {
        const response = await fetch(
          `${API_URL}/webhooks${event.apiEndpoint}`,
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
              [`${eventType}_webhook`]: webhookUrls[eventType],
            }),
          }
        );
        if (response.ok) {
  ...
  return (
    <Card>
      ...
      <Text size="sm" mb="lg">
        ...
        happen. Below, you can set up and manage webhook URLs for different
        events.
      </Text>
      ...
      <Box mt="xs" mb="xs">
        <Collapse in={showInstructions}>
          <Paper shadow="xs" p="md" withBorder>
            ...
            specific events. When such an event occurs, the source site makes an
            HTTP request to the URL configured for the webhook. This allows for
            real-time data transmission between applications.
          </Paper>
        </Collapse>
        ...
        {eventTypes.map((eventType) => (
          <Card key={eventType.value} shadow="sm" p="lg" withBorder>
            <Flex align="center" justify="space-between" mb="md">
              <Box>
                <Text weight={500}>
                  {eventType.label} {eventType.disabled && "(Coming soon ⚠️)"}
                </Text>
